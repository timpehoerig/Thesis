I have implemented the type inference algorithm for Featherweight Generic Java as describe in the paper\cite{FGJ} given by Stadelmeier et al.
...
yields one solution after another. From outside \inl{Unify} behaves as it is describe in the paper. However, the implementation slightly
...
In order to make the algorithm usable for more than one class I changed the function \inl{TypeInference} to not only considers one class but a whole
