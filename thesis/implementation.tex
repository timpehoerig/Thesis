...
A pair of constraints ($\nvtt{N}{T} < \fa{}$, $\fa{} < \nvtt{M}{U}$) can only be solved if $\nvtt{N}{T}$ is a subtype of $\nvtt{M}{U}$. Hence this subtyping constraint is added to the constraint set. After applying the rule of figure .. exhaustively to the constraint set the constraint set is either in solved form or no solution exists.
Every possible $\nvtt{N}{V}$ between $\nvtt{N}{T}$ and $\nvtt{M}{U}$
\\
\\
Having constraints such that (a < $\nvt{C}{T}$) and (a <* b) so it is possible that ($\nvt{C}{T}$ < b) is implied is not possible. because the dual rules of DOES IT?
...
Before we go on we need to transform all type variables to parameterless classes ($\text{X} \to \nvtt{X}{}$).
...
No further rule applies and we can go to step 6. Before we start with step 6 we need to transform the type variables from parameterless classes back to normal type variables (for example: $\nvtt{X}{} \to \text{X}$). Now we can start with step 6. We first divide the constraint set into sub- and equal-constraints resulting in the following:
...
For the sub-constraint we introduce a new type variable \inl{Z} with its upper bound determined by the sub-constraint.
...
Here we have more than just one class
